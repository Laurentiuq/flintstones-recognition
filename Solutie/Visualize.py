...
def show_detections_without_ground_truth(detections, scores, file_names, task, params: Parameters):
    """
    ...
    for test_file in test_files:
        ...
        cv.imwrite(os.path.join(params.dir_save_files, "detections_" + short_file_name + task), image)
        print('Apasa orice tasta pentru a continua...')
        cv.imshow('image', np.uint8(image))
        cv.waitKey(0)


def show_detections_with_ground_truth(detections, scores, file_names, task, params: Parameters):
    """
    ...
    for test_file in test_files[:params.nr_test]:
        image = cv.imread(test_file)
        short_file_name = ntpath.basename(test_file)
        indices_detections_current_image = np.where(file_names == short_file_name)
        current_detections = detections[indices_detections_current_image]
